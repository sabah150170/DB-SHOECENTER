...
def my_basket():
	if request.method=='POST':
		...
		cursor.execute("SELECT * FROM CUSTOMER_INFO WHERE (CUSTOMER_ID = %s)", (cid,))
		info=cursor.fetchall()

		if quantity_basket[0][0] == 0:
			return render_template("customer_welcome.html", notice="No shoes in your basket!", customer_id=cid, info=info)

		else:
			cursor.execute("SELECT * FROM (SELECT * FROM ((SELECT * FROM BASKET) as n1 left join (SELECT * FROM STOCK_INFO) as n2 using(STOCK_ID)) as n3 inner join (SELECT RATE_STORE,STORE_ID from STORE_INFO) as n4 using(STORE_ID)) as nw5 left join (SELECT * FROM SHOE_INFO) as n6 using(SHOE_ID) where customer_id=%s",(cid,))
			basket=cursor.fetchall()

			return render_template("customer_welcome.html", basket=basket, customer_id=cid, info=info, size_basket=len(basket))
...
def my_stock():
	if request.method=='POST':
		...
		cursor.execute("SELECT * FROM STORE_INFO WHERE(STORE_ID=%s)",(sid,))
		info=cursor.fetchall()

		cursor.execute("SELECT * FROM ADDRESS WHERE(ADDRESS_ID=%s)",(info[0][1],))
		address=cursor.fetchall()
		...
		if len(result)==0:		
			notice+="  \n No stock "		
			return render_template("company_welcome.html", company_id=sid, notice=notice, info=info[0][5], address=address)

		else:			
			return render_template("company_welcome.html", company_id=sid, notice=notice, result3=result, size=len(result),info=info[0][5], address=address)


@app.route("/customer_find", methods=['POST'])
def customer_find():
	if request.method=='POST':
		sid=request.form['customer_id']
		rate_shoe=int(request.form['rate_shoe'])
		number_brand=int(request.form['n_brand'])
		number_feature=int(request.form['n_feature'])
		number_color=int(request.form['n_color'])
...
def buy_shoes():
	if request.method=='POST':   
		...
		cursor.execute("SELECT COUNT(STOCK_ID) FROM STOCK_INFO WHERE STOCK_ID=%s",(stock_id,))
		quantity_s=cursor.fetchall()		

		if quantity_s[0][0]==1:

			cursor.execute("SELECT COUNT(BASKET_ID) FROM BASKET WHERE STOCK_ID=%s and CUSTOMER_ID=%s",(stock_id,customer_id))
			quantity_b=cursor.fetchall()

			if quantity_b[0][0]==0:
				cursor.execute("INSERT INTO BASKET (STOCK_ID, CUSTOMER_ID, QUANTITY) VALUES (%s, %s, %s)", (stock_id, customer_id, quantity ))
				DB.commit()
			else:
